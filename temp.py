...
s  = stocks.index('all') #choose desired stock from list
n  = datax.shape[0]
...
#auch mit if else möglich  
#Ergebnisse sind noch verschoben, checken mit R 
for i in range(shp_x): #müsste nur für Linde sein & nicht für alle zusammen
  
    dat1 = np.log(x.iloc[1:, i]).reset_index()
    dat2 = pd.DataFrame(np.log(x.iloc[0:(n - 1), i]))
    ret1 = pd.DataFrame(dat1.iloc[:,1] - dat2.iloc[:,0])
